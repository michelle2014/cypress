...
import { VisualRegressionResult } from 'cypress-visual-regression'
...
describe('Visual Regression Example', () => {
  ...
  it('should handle custom error thresholds correctly', () => {
    if (Cypress.env('visualRegressionType') === 'base') {
      ...
      cy.visit('./cypress/web/05.html')
      cy.get('H1').contains('none').should('exist')
      cy.compareSnapshot('foo', 0.03).should((result: VisualRegressionResult) => {
        expect(result.error).to.be.undefined
        expect(result.percentage).to.be.below(0.03)
      })
      ...
      cy.get('H1')
        .compareSnapshot('h1', 0.1)
        .should((result: VisualRegressionResult) => {
          expect(result.error).to.be.undefined
          expect(result.percentage).to.be.below(0.1)
        })
      cy.get('H1')
